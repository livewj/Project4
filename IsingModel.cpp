...
int main(int argc, char* argv[])
{
  ...
  ofile << "      T:            <E>/L^2:     Evariance/T^2:  <M>/L^2:      Mvariance/T:     <|M|>/L^2:    MCcycles:     Accept:" << endl;

  //Comment/uncomment this section
  // 2c) Start Monte Carlo sampling by looping over the selcted Temperatures
  for (double Temperature = InitialTemp; Temperature <= FinalTemp; Temperature+=TempStep){
    ...
    // 
    WriteResultstoFile(NSpins, MCcycles, Temperature, ExpectationValues, accept);
  }
  ofile.close();  // close output file

/*
  //Comment/uncomment this section
  double Temperature = 2.4; //fixed temp

    //2c) Start Monte Carlo sampling by looping over different Monte Carlo cycles
  for (double cycles = 10; cycles <= MCcycles; cycles*=1.1){
    vec ExpectationValues = zeros<mat>(5);
    int accept = 0;
    // Start Monte Carlo computation and get expectation values
    MetropolisSampling(NSpins, cycles, Temperature, ExpectationValues,accept);
    // 
...
void WriteResultstoFile(int NSpins, int MCcycles, double temperature, vec ExpectationValues, int accept)
{
